...
const features = [
  {
    ...
    link: (<a href="Przejdź do quizu" key="" title="quiz"></a>) as any,
    // image: feature1,
    video: "/feature_1.mp4",
  },
  {
    id: 2,
    title: "Udostępnij kalendarz",
    description:
...
export default function AccordionFeatures({ config }: { config?: any }) {
  const { open } = useDrawerStore() as any;
          ...
          <div className="grid grid-cols-1 items-stretch gap-8 sm:gap-12 lg:grid-cols-2 lg:gap-24">
            <ul className="w-full">
              {features.map((feature) => (
                ...
                >
                  <button
                    className="relative flex w-full items-center gap-2 py-4 text-left text-xl font-medium md:text-2xl"
            ...
            <div className="">
              {features.map((feature) => (
                ...
                <div
                  key={feature.id}
                  className={`w-full rounded-[16px] drop-shadow-lg border bg-slate-50 object-contain object-center transition-opacity duration-300 sm:-m-2 sm:w-[26rem] sm:p-2 ${
                    activeFeature === feature.id
                      ? "opacity-100"
                      : "hidden opacity-0"
                  }`}
